...
describe("週の学習時間割(suggestTimetable)", () => {
  ...
  it("過去問は週末(土日)の午前に入り、平日には入らない", () => {
    const g = L.suggestTimetable(planned, []); // 3コマ未消化
    const weekend = [...g[5], ...g[6]];
    expect(weekend.filter((x) => L.isKakomon(x)).length).toBeGreaterThan(0);
    const weekday = [...g[0], ...g[1], ...g[2], ...g[3], ...g[4]];
    expect(weekday.filter((x) => L.isKakomon(x)).length).toBe(0);
  });
  it("時間割の過去問は特定年度を持つ(一括りにしない)", () => {
    const g = L.suggestTimetable(planned, []);
    const kk = g.flat().filter((x) => L.isKakomon(x));
    expect(kk.length).toBeGreaterThan(0);
    // 「過去問 2023①」のように年度を含む
    for (const v of kk) expect(v).toMatch(/過去問 20\d\d/);
    // 古い年度から(2024①が先)
    expect(kk[0]).toContain("2024");
  });
