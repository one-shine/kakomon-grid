...
export function AttemptForm() {
  const { schoolId, attemptId, prefill } = useNav();
  const nav = useNav();
  ...
  const [year, setYear] = useState<number>(editing ? editing.year : prefill ? prefill.year : thisYear);
  const [round, setRound] = useState(editing?.round ?? prefill?.round ?? "");
